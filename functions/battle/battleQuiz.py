...
def generate_multiple_choice_quiz_with_check(summary, previous_quiz=None) -> Tuple[str, int]:
    ...
    주어진 아티클 요약을 기반으로 5지 선다형 객관식 문제를 **1개** 생성하세요.
    ...
    ## 주의 사항 
        - {f"이전 문제와 동일한 문제를 출제하지 않도록 주의해주세요. 이전 문제: {previous_quiz}" if previous_quiz else ""}
        - 퀴즈의 구성 내용은 매번 바뀌게 출력한다. previous_quiz와 같으면 **절대** 안 됩니다. 
        - 첫 번째 문제와 중복되지 않도록 해야 하며, 두 번째 문제는 완전히 다른 질문이어야 합니다.
        - **한 개**의 퀴즈만 생성합니다.
        - 정답을 화면에 절대 출력하지 않도록 합니다. 출력 형식에 맞게 생성합니다.
